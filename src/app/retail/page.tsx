    ...
    <section className="relative h-screen overflow-hidden bg-black text-white">
          ...
          <div className="absolute inset-0 pointer-events-none z-20">
            {particles.map((p, i) => (
              <motion.span
                key={i}
                className="absolute w-0.5 h-0.5 bg-[#c38a27] rounded-full opacity-60"
                style={{ top: p.y, left: p.x }}
                animate={{ y: [p.y, p.y - 40, p.y], opacity: [0.3, 1, 0.3], scale: [1, 1.5, 1] }}
...
function CoreCapabilities() {
  const items = [
    ...
    { icon: <Zap size={36} />, label: "Interactive Lighting & Displays" },         // energy, interactivity, tech-driven
  ];
      ...
      <div className="max-w-4xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} viewport={{ once: true }}>
          ...
          <p className="text-xl md:text-2xl font-semibold text-gray-300 leading-relaxed mb-6">
            “Our store now tells a story — one our customers love to be part of.”
          </p>
    ...
    >
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
        <h2 className="text-4xl md:text-5xl font-bold mb-6">
          Let’s Redefine  <span className={COPPER_GRADIENT}>Retail</span> Together
        </h2>
